      ...
      <div className="flex-1 flex flex-col">
        {activeUrl ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-muted-foreground">Sinal indisponível.</p>
          </div>
        )}
          ...
          {video.type === 'series' && video.seasons && (
            <div className="space-y-6">
              <div className="flex items-center gap-4 border-b border-border pb-4 overflow-x-auto">
                {video.seasons.map(s => (
                  <button key={s.id} className={`text-lg font-bold pb-4 -mb-4 ${selectedSeason === s.number ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`} onClick={() => setSelectedSeason(s.number)}>
                    Temp {s.number}
                  </button>
                ))}
              </div>
              <div className="grid gap-2">
                {video.seasons.find(s => s.number === selectedSeason)?.episodes.map(ep => (
                  <button key={ep.id} onClick={() => setActiveUrl(ep.url)} className={`flex items-center gap-4 p-4 rounded-xl border ${activeUrl === ep.url ? 'bg-primary/10 border-primary' : 'bg-card'}`}>
                    <Play className={`w-4 h-4 ${activeUrl === ep.url ? 'text-primary fill-primary' : ''}`} />
